...
var template = '/*! p.%ext% build:' + package.version + ', %type%. Copyright(c) 2013 Eric Zhang <eric@ericzhang.com> */'
  , prefix = '\n(function(exports){\n'
...
if (!module.parent){
  ...
  builder(args.length ? args : false, { minify:false }, function (err, content) {
    ...
    fs.write(
        fs.openSync(__dirname + '/../dist/p.js', 'w')
      , content
      , 0
      , 'utf8'
    );
    console.log('Successfully generated the development build: p.js');
  });
  ...
  builder(args.length ? args : false, function (err, content) {
    ...
    fs.write(
        fs.openSync(__dirname + '/../dist/p.min.js', 'w')
      , content
      , 0
      , 'utf8'
    );
    console.log('Successfully generated the production build: p.min.js');
  });
}
